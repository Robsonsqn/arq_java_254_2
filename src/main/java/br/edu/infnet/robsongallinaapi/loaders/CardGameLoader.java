...
@Component
@Order(3)
public class CardGameLoader implements ApplicationRunner {

    private final GameService gameService;
    private final PublisherService publisherService;

    @Value("${game.data.loader.path.cardgames}")
    private Resource resourceFile;

    public CardGameLoader(GameService gameService, PublisherService publisherService) {
    ...
    public void run(ApplicationArguments args) throws Exception {
        System.out.println(">>> Loading CardGame data from: " + resourceFile.getFilename());
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(resourceFile.getInputStream(), StandardCharsets.UTF_8))) {
            reader.lines().forEach(line -> {
                ...
                CardGame cardGame = new CardGame(null, data[0], data[1], Integer.parseInt(data[2]),
                        Boolean.parseBoolean(data[3]), Boolean.parseBoolean(data[4]),
                        Integer.parseInt(data[7]), data[8], publisher);
                gameService.create(cardGame);
            });
        }
        System.out.println(">>> CardGame data loaded.");
    }
}