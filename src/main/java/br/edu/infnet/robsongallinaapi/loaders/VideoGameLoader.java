...
@Component
@Order(1)
public class VideoGameLoader implements ApplicationRunner {

    private final GameService gameService;
    private final PublisherService publisherService;

    @Value("${game.data.loader.path.videogames}")
    private Resource resourceFile;

    public VideoGameLoader(GameService gameService, PublisherService publisherService) {
    ...
    public void run(ApplicationArguments args) throws Exception {
        System.out.println(">>> Loading VideoGame data from: " + resourceFile.getFilename());
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(resourceFile.getInputStream(), StandardCharsets.UTF_8))) {
            reader.lines().forEach(line -> {
                ...
                VideoGame videoGame = new VideoGame(null, data[0], data[1], Integer.parseInt(data[2]),
                        Boolean.parseBoolean(data[3]), Boolean.parseBoolean(data[4]), data[7], data[8], publisher);
                gameService.create(videoGame);
            });
        }
        System.out.println(">>> VideoGame data loaded.");
    }
}